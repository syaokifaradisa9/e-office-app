...
export default function VisitorSidebar() {
    ...
    return (
        <div className="mb-6 space-y-6">
            ...
            {/* Group 1: Data Pengunjung */}
            {(permissions?.includes('lihat_data_pengunjung') || permissions?.includes('konfirmasi_kunjungan') || permissions?.includes('buat_undangan_tamu')) && (
                <div className="space-y-1">
                    <div className="py-2">
                        <h3 className="visitor-sidebar-label text-[10px] font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">Data Pengunjung</h3>
                    </div>
            ...
            {(permissions?.includes('lihat_master_manajemen_pengunjung') || permissions?.includes('kelola_master_manajemen_pengunjung') ||
                permissions?.includes('lihat_pertanyaan_feedback') || permissions?.includes('kelola_pertanyaan_feedback')) && (
                    <div className="space-y-1">
                        <div className="py-2">
                            <h3 className="visitor-sidebar-label text-[10px] font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">Master Data Pengunjung</h3>
                        </div>
                        {(permissions?.includes('lihat_master_manajemen_pengunjung') || permissions?.includes('kelola_master_manajemen_pengunjung')) && (
            ...
            {permissions?.includes('lihat_laporan_pengunjung') && (
                <div className="space-y-1">
                    <div className="py-2">
                        <h3 className="visitor-sidebar-label text-[10px] font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">Laporan Pengunjung</h3>
                    </div>
